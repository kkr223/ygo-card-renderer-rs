//! Secret Rare weave and micro-foil effects.
//!
//! `draw_secret_weave` — full "#" diffraction pattern for GSeR illustration.
//! `draw_secret_foil` — simplified version for small masked regions.
...
pub(crate) fn draw_secret_weave(target: &mut Pixmap, rect: CoverageRect, opacity: f32) {
    ...
    for y in rect.y.min(height)..y_end {
        for x in rect.x.min(width)..x_end {
            ...
            let cool_temp_hue = 0.535 + fine_hash * 0.115;
            let warm_temp_hue = 0.030 + fine_hash * 0.075;
            ...
            let (r, g, b) = if spark_on {
                ...
                let (glow_r, glow_g, glow_b) = crate::pixel_ops::hsv_to_rgb(
                    glow_hue,
