...
/// Secret Collector's Rare foil: diagonal-elongated micro-facet cells (most
/// cells are long bottom-left→top-right strips, a few are dots), dual-grating
/// diffraction, and several diagonal interference fringes instead of the "#" 
/// band pattern used by SER.
pub(crate) fn draw_optical_scr(target: &mut Pixmap, rect: CoverageRect, opacity: f32) {
    ...
    // Diagonal fringe parameters
    let fringe_pitches = [0.18_f32, 0.32, 0.55, 0.78];
    let fringe_angles = [-0.72_f32, -0.58, -0.45, -0.36]; // radians, diagonal tilt
    let fringe_widths = [0.032_f32, 0.038, 0.028, 0.042];
    ...
    for y in y_start..y_end {
        for x in x_start..x_end {
            ...
            // ── Diagonal-elongated micro-facet ───────────────────────────────
            let (nx, ny, nz, inside) = scr_micro_facet(local_x, local_y, params);
            let n_dot_l = clamp01(nx * light.0 + ny * light.1 + nz * light.2);
            let n_dot_h = clamp01(nx * half_vec.0 + ny * half_vec.1 + nz * half_vec.2);

            // Dual grating (same core as SER)
            let light_mod = (n_dot_l - 0.5) * params.light_blaze_shift;
            let delta1 = nx * params.tilt_factor + params.blaze1 + light_mod;
            let delta2 = ny * params.tilt_factor + params.blaze2 - light_mod;
            let order = params.diffraction_order.max(1) as f32;
            ...
            // ── Diagonal interference fringes ────────────────────────────────
            let mut fringe_energy = 0.0_f32;
            for i in 0..fringe_pitches.len() {
                let angle = fringe_angles[i];
                let proj = u * angle.cos() + v * angle.sin();
                let wave = (proj / fringe_pitches[i] * std::f32::consts::TAU).sin();
                let fringe = (wave * 0.5 + 0.5).powf(2.5);
                let g = (-(proj - (i as f32 + 1.0) * 0.2).powi(2)
                    / (2.0 * fringe_widths[i] * fringe_widths[i]))
                    .exp();
                fringe_energy += fringe * g * 0.55;
            }
            fringe_energy = fringe_energy.clamp(0.0, 1.0);
            let fringe_mod = 0.25 + 0.75 * fringe_energy;
            let modulated_energy = combined_energy * fringe_mod;

            // Diffraction colour with fringe modulation
            let micro_scale2 = modulated_energy * params.micro_gain * 0.5;
            let fringe_micro_r = (rgb1.0 + rgb2.0) * micro_scale2;
            let fringe_micro_g = (rgb1.1 + rgb2.1) * micro_scale2;
            let fringe_micro_b = (rgb1.2 + rgb2.2) * micro_scale2;

            let diff_r =
                1.0 - (-(micro_r * 0.30 + fringe_micro_r * 0.70) * params.foil_gain).exp();
            let diff_g =
                1.0 - (-(micro_g * 0.30 + fringe_micro_g * 0.70) * params.foil_gain).exp();
            let diff_b =
                1.0 - (-(micro_b * 0.30 + fringe_micro_b * 0.70) * params.foil_gain).exp();

            // Simplified specular
            let spec =
                n_dot_h.powf(params.shininess) * combined_energy * 0.7;
            ...
            let darken = 0.50;
            let result_r = clamp01(
                base_r * darken + diff_r + spec * params.white_gain * 0.6,
            );
            let result_g = clamp01(
                base_g * darken + diff_g + spec * params.white_gain * 0.6,
            );
            let result_b = clamp01(
                base_b * darken + diff_b + spec * params.white_gain * 0.6,
            );
...
/// Simplified SCR foil for small masked regions (attribute icons, level/rank
/// stars, link arrows). Same diagonal-elongated micro-facet and fringe core,
/// but with a left-to-right warm→cool gradient sweep.
pub(crate) fn draw_optical_scr_simple(target: &mut Pixmap, rect: CoverageRect, opacity: f32) {
    ...
    for y in y_start..y_end {
        for x in x_start..x_end {
            ...
            let spec = n_dot_h.powf(params.shininess) * combined_energy * 0.5;
            ...
            let foil_strength = inside * (0.20 + n_dot_l * 0.60 + n_dot_h * 0.20);
            let mix_gradient = 0.35 + foil_strength * 0.30;
            let mut foil_rgb = lerp_rgb(
                (diff_r, diff_g, diff_b),
                gradient_rgb,
                mix_gradient.min(0.92),
            );
            ...
            let temp_flip = smoothstep(0.64, 0.965, temp_hash) * foil_strength;
            let warm_bias =
                smoothstep(0.58, 0.90, foil_rgb.0 + foil_rgb.1 * 0.35 - foil_rgb.2 * 0.25);
            let fine_hash = ((pin_hash >> 11) & 0xff) as f32 / 255.0;
            let opposite_hue = if warm_bias > 0.45 {
                0.600 + fine_hash * 0.090
            } else {
...
/// Micro-facet normal map with diagonal-elongated cells.
/// Most cells are long strips oriented bottom-left→top-right (45°);
/// a random subset (~15%) are compact dots.
fn scr_micro_facet(
    local_x: u32,
    local_y: u32,
    params: OpticalSerParams,
    ...
    // Rotate to diagonal space: +45° maps bottom-left→top-right to horizontal
    let cos45 = std::f32::consts::FRAC_1_SQRT_2;
    let sin45 = cos45;
    let rx = xf * cos45 + yf * sin45;
    let ry = -xf * sin45 + yf * cos45;

    // Cell dimensions in rotated space (long along rx = diagonal)
    let cell_w: i32 = 16;
    let cell_h: i32 = 5;
    let gap: i32 = 4;
    let pitch_x = (cell_w + gap).max(1);
    let pitch_y = (cell_h + gap).max(1);

    let col = (rx / pitch_x as f32).floor() as i32;
    let row = (ry / pitch_y as f32).floor() as i32;

    // Hash for cell properties
    let cell_hash = |c: i32, r: i32, s: u32| -> f32 {
        let mut x = (c as u32).wrapping_mul(1_664_525).wrapping_add(s)
            ^ (r as u32).wrapping_mul(1_013_904_223);
        x ^= x >> 16;
        x = x.wrapping_mul(2_246_822_519);
        x ^= x >> 13;
        x = x.wrapping_mul(3_266_489_917);
    ...
    // ~15% of cells are compact dots instead of long strips
    let is_dot = cell_hash(col, row, params.seed + 7777) > 0.85;

    let (cell_ew, cell_eh) = if is_dot {
        (cell_w as f32 * 0.35, cell_h as f32 * 0.7)
    } else {
        (cell_w as f32, cell_h as f32)
    };

    let cx = (col as f32 + 0.5) * pitch_x as f32;
    let cy = (row as f32 + 0.5) * pitch_y as f32;
    let dx = (rx - cx) / (cell_ew * 0.5);
    let dy = (ry - cy) / (cell_eh * 0.5);

    // Use superellipse exponent for rectangular feel on strips
    let x_pow = if is_dot { 2.5_f32 } else { 4.5_f32 };
    let r = (dx.abs().powf(x_pow) + dy.abs().powf(2.0)).powf(1.0 / x_pow.max(2.0));
    let inside = 1.0 - smoothstep(0.82, 1.06, r);

    let tilt_x = (cell_hash(col, row, params.seed + 1337) - 0.5) * 2.0 * params.tilt_strength;
    let tilt_y = (cell_hash(col, row, params.seed + 8128) - 0.5) * 2.0 * params.tilt_strength;
    let r_dist = (dx * dx + dy * dy).sqrt();
    let slope = params.dome_depth * r_dist.clamp(0.0, 1.0);
    let active = if inside > 0.001 { 1.0 } else { 0.0 };
    let nx_local = (-dx * slope + tilt_x) * active;
    let ny_local = (-dy * slope + tilt_y) * active;

    // Rotate normal back from diagonal space to card space
    let nx = nx_local * cos45 - ny_local * sin45;
    let ny = nx_local * sin45 + ny_local * cos45;
    let inv_len = 1.0 / (nx * nx + ny * ny + 1.0).sqrt();
    (nx * inv_len, ny * inv_len, inv_len, inside)
}
